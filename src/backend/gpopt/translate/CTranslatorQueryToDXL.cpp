	...
	)
{
	...
	ListCell *lcWindowSpec = NULL;
	ForEach (lcWindowSpec, window_clause)
	{
		WindowSpec *wspec = (WindowSpec *) lfirst(lcWindowSpec);
		ULongPtrArray *part_columns = TranslatePartColumns(wspec->partition, sort_col_attno_to_colid_mapping);
		...
		if (NULL != wspec->name)
		{
			CWStringDynamic *alias_str = CDXLUtils::CreateDynamicStringFromCharArray(m_mp, wspec->name);
			mdname = GPOS_NEW(m_mp) CMDName(m_mp, alias_str);
			GPOS_DELETE(alias_str);
		}

		if (0 < gpdb::ListLength(wspec->order))
		{
			// create a sorting col list
			sort_col_list_dxl = GPOS_NEW(m_mp) CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarSortColList(m_mp));

			CDXLNodeArray *dxl_sort_cols = TranslateSortColumsToDXL(wspec->order, sort_col_attno_to_colid_mapping);
		...
		if (NULL != wspec->frame)
		{
			window_frame = m_scalar_translator->GetWindowFrame((Expr *) wspec->frame, m_var_to_colid_map, project_list_dxlnode_node, &m_has_distributed_tables);
		}
