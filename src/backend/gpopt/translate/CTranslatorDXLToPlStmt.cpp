	...
	)
{
	...
	ForEach (plcLocation, extentry->urilocations)
	{
		...
		Uri *pUri = gpdb::ParseExternalTableUri(szUri);
	...
	)
{
	...
	{
		CHAR *pcEnd = NULL;
		INT iTargetSegInd = (INT) gpos::clib::Strtoll(szOnClause + gpos::clib::Strlen("SEGMENT_ID:"), &pcEnd, 10);
	...
	else if (0 == gpos::clib::Strncmp(szOnClause, "TOTAL_SEGS:", gpos::clib::Strlen("TOTAL_SEGS:")))
	{
		...
		CHAR *pcEnd = NULL;
		ULONG ulSegsToUse = gpos::clib::Strtoll(szOnClause + gpos::clib::Strlen("TOTAL_SEGS:"), &pcEnd, 10);
	...
	)
{
	...
	if (!fUsingExecute)
	{
		...
		pUri = gpdb::ParseExternalTableUri(szFirstUri);
	}
	...
	)
{
	...
	if (IMDRelation::ErelstorageExternal == md_rel->RetrieveRelStorageType())
	{
		...
		Value *pval = gpdb::MakeStringValue(ext_table_entry->fmtopts);
		ext_scan->fmtOpts = ListMake1(pval);
	...
	)
{
	...
	for (ULONG ul = 0; ul < arity; ul++)
	{
		...
		gpdb::IndexOpProperties(cmp_operator_oid, op_family_oid, &strategy_num, &index_subtype_oid, &fRecheck);
	...
	)
{
	...
	for (ULONG ul = 0; ul < size; ul++)
	{
		...
		TranslateSortCols(sort_col_list_dxlnode, &child_context, pwindowkey->sortColIdx, pwindowkey->sortOperators, pwindowkey->nullsFirst);

		// translate the window frame specified in the window key
		pwindowkey->frame = NULL;
		if (NULL != window_key->GetWindowFrame())
		{
			pwindowkey->frame = Pwindowframe(window_key->GetWindowFrame(), &child_context, output_context, plan);
		}
		window->windowKeys = gpdb::LAppend(window->windowKeys, pwindowkey);
	}
	...
	)
{
	...
	pwindowframe->exclude = CTranslatorUtils::Windowexclusion(pdxlwf->ParseFrameExclusionStrategy());

	// translate the CDXLNodes representing the leading and trailing edge
	CDXLTranslationContextArray *pdrgpdxltrctx = GPOS_NEW(m_mp) CDXLTranslationContextArray(m_mp);
	...
	pwindowframe->lead->kind = CTranslatorUtils::Windowboundkind(CDXLScalarWindowFrameEdge::Cast(pdxlnLead->GetOperator())->ParseDXLFrameBoundary());
	pwindowframe->lead->val = NULL;
	if (0 != pdxlnLead->Arity())
	{
		pwindowframe->lead->val = (Node*) m_translator_dxl_to_scalar->TranslateScalarExprFromDXL((*pdxlnLead)[0], &mapcidvarplstmt);
	}


	CDXLNode *pdxlnTrail = pdxlwf->PdxlnTrailing();
	pwindowframe->trail = MakeNode(WindowFrameEdge);
	pwindowframe->trail->kind = CTranslatorUtils::Windowboundkind(CDXLScalarWindowFrameEdge::Cast(pdxlnTrail->GetOperator())->ParseDXLFrameBoundary());
	pwindowframe->trail->val = NULL;
	if (0 != pdxlnTrail->Arity())
	{
		pwindowframe->trail->val = (Node*) m_translator_dxl_to_scalar->TranslateScalarExprFromDXL((*pdxlnTrail)[0], &mapcidvarplstmt);
	}
	...
	)
{
	...
	GpPolicy *distr_policy = (GpPolicy *) gpdb::GPDBAlloc(sizeof(GpPolicy) +
															num_of_distr_cols_alloc * sizeof(AttrNumber));
	...
	)
{
	...
	bitmap_idx_scan->logicalIndexInfo = gpdb::GetLogicalIndexInfo(oidRel, index_oid);
