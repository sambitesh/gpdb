	...
	)
{
	...
	{
		PercentileExpr *ppercentile = (PercentileExpr*) node;
	...
	)
{
	...
	IntoClause *into_clause = derived_table_query->intoClause;
	...
	)
{
	...
	{
		SortClause *sort_clause  = (SortClause*) lfirst(pl);
		GPOS_ASSERT(NULL != sort_clause);

		GroupClause *group_clause = MakeNode(GroupClause);
		group_clause->tleSortGroupRef = sort_clause->tleSortGroupRef;
		group_clause->sortop = sort_clause->sortop;
		group_clause->nulls_first = sort_clause->nulls_first;
		new_query->groupClause = gpdb::LAppend(new_query->groupClause, group_clause);
	}
