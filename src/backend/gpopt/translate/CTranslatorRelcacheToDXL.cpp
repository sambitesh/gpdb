	...
	)
{
	...
	if (0 == num_of_levels && gpdb::HasSubclass(rel_oid))
	{
	...
	)
{
	...
	if (IMDRelation::ErelstorageExternal == rel_storage_type)
	{
		...
		IMDId *mdid_format_err_tbl = NULL;
		if (InvalidOid != extentry->fmterrtbl)
		{
			mdid_format_err_tbl = GPOS_NEW(mp) CMDIdGPDB(extentry->fmterrtbl);
		}
	...
	)
{
	...
	GPOS_TRY
	{
		...
		if (BITMAP_AM_OID == index_rel->rd_rel->relam || IMDRelation::ErelstorageAppendOnlyRows == rel_storage_type || IMDRelation::ErelstorageAppendOnlyCols == rel_storage_type)
		{
	...
	)
{
	...
	GPOS_ASSERT(INDTYPE_BITMAP == index_info->indType || INDTYPE_BTREE == index_info->indType);
	...
	)
{
	...
	for (ULONG ul = 0; ul < num_mcv_values; ul++)
	{
		...
		IDatum *datum = CTranslatorScalarToDXL::GetDatum(mp, md_type, false /* is_null */, datumMCV);
		datums->Append(datum);
	...
	)
{
	...
	for (ULONG ul = 0; ul < num_buckets; ul++)
	{
		IDatum *min_datum = CTranslatorScalarToDXL::GetDatum(mp, md_type, false /* is_null */, hist_values[ul]);
		IDatum *max_datum = CTranslatorScalarToDXL::GetDatum(mp, md_type, false /* is_null */, hist_values[ul + 1]);
	...
	)
{
	...
	*part_types = GPOS_NEW(mp) CharPtrArray(mp);
	...
	PartitionNode *partition = gpdb::GetParts(oid, 0 /*level*/, 0 /*parent*/, false /*inctemplate*/, true /*includesubparts*/);
	GPOS_ASSERT(NULL != partition);
	
	if (gpdb::FHashPartitioned(partition->part->parkind))
	{
	...
	)
{
	...
	return gpdb::HeapAttIsNull(tup, Anum_pg_index_indexprs) &&
		...
		(BTREE_AM_OID == index_rel->rd_rel->relam || BITMAP_AM_OID == index_rel->rd_rel->relam);
}
