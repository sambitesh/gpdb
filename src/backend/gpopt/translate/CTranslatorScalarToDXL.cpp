	...
	)
{
	static const STranslatorElem translators[] =
	{
		...
		{T_WindowRef, &CTranslatorScalarToDXL::CreateScalarWindowRef},
	...
	)
{
	...
	CDXLNode *left_node = CreateScalarOpFromExpr(left_expr, var_colid_mapping);
	CDXLNode *right_node = CreateScalarOpFromExpr(right_expr, var_colid_mapping);
	...
	)
{
	...
	CDXLDatum *datum_dxl = CTranslatorScalarToDXL::GetDatumVal(mp, md_type, constant->consttypmod, constant->constisnull,
															 constant->constlen,
															 constant->constvalue);
...
CTranslatorScalarToDXL::GetDatumVal
	...
	const
{
	return GetDatumVal(m_mp, m_md_accessor, constant);
}
	...
	)
{
	...
	CDXLNode *dxlnode_arg = CreateScalarOpFromExpr(boolean_test->arg, var_colid_mapping);
	...
	)
{
	...
	CDXLNode *child_node = CreateScalarOpFromExpr(null_test->arg, var_colid_mapping);
	...
	)
{
	...
	ForEach (lc, list)
	{
		...
		CDXLNode *child_node = CreateScalarOpFromExpr(child_expr, var_colid_mapping, has_distributed_tables);
		GPOS_ASSERT(NULL != child_node);
		dxlnode->AddChild(child_node);
	}
}
	...
	)
{
	...
	CDXLNode *dxlnode_arg = CreateScalarOpFromExpr(case_expr->arg, var_colid_mapping);
	...
	ForEach (lc, case_expr->args)
	{
		...
		CDXLNode *cmp_expr_node = CreateScalarOpFromExpr(expr->expr, var_colid_mapping);
		GPOS_ASSERT(NULL != cmp_expr_node);

		CDXLNode *result_node = CreateScalarOpFromExpr(expr->result, var_colid_mapping);
	...
	{
		CDXLNode *default_result_node = CreateScalarOpFromExpr(case_expr->defresult, var_colid_mapping);
	...
	)
{
	...
	for (ULONG ul = 0; ul < when_clause_count; ul++)
	{
		...
		CDXLNode *cond_node = CreateScalarOpFromExpr(expr->expr, var_colid_mapping);
		CDXLNode *result_node = CreateScalarOpFromExpr(expr->result, var_colid_mapping);
	...
	)
{
	...
	CDXLNode *child_node = CreateScalarOpFromExpr(relabel_type->arg, var_colid_mapping);
        ...
        )
{
        ...
        CDXLNode *child_node = CreateScalarOpFromExpr(coerce->arg, var_colid_mapping);
	...
	)
{
	...
	CDXLNode *child_node = CreateScalarOpFromExpr(array_coerce_expr->arg, var_colid_mapping);
	...
	)
{
	...
	const WindowFrame *window_frame = (WindowFrame *) expr;

	EdxlFrameSpec frame_spec = EdxlfsRow;
	if (!window_frame->is_rows)
	{
		frame_spec = EdxlfsRange;
	}

	EdxlFrameBoundary edxlfbLead = Edxlfb(window_frame->lead->kind, window_frame->lead->val);
	EdxlFrameBoundary edxlfbTrail = Edxlfb(window_frame->trail->kind, window_frame->trail->val);
	...
	{
		ULONG *elem = rgrgulExclusionMapping[ul];
		if ((ULONG) window_frame->exclude == elem[0])
		{
			strategy = (EdxlFrameExclusionStrategy) elem[1];
			break;
		}
	}
	GPOS_ASSERT(EdxlfesSentinel != strategy && "Invalid window frame exclusion");

	CDXLNode *lead_edge = GPOS_NEW(m_mp) CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarWindowFrameEdge(m_mp, true /* fLeading */, edxlfbLead));
	CDXLNode *trail_edge = GPOS_NEW(m_mp) CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarWindowFrameEdge(m_mp, false /* fLeading */, edxlfbTrail));

	// translate the lead and trail value
	if (NULL != window_frame->lead->val)
	{
		lead_edge->AddChild(CreateDXLNodeFromWindowFrameEdgeVal(window_frame->lead->val, var_colid_mapping, new_scalar_proj_list, has_distributed_tables));
	}

	if (NULL != window_frame->trail->val)
	{
		trail_edge->AddChild(CreateDXLNodeFromWindowFrameEdgeVal(window_frame->trail->val, var_colid_mapping, new_scalar_proj_list, has_distributed_tables));
	}
...
{
	CDXLNode *val_node = CreateScalarOpFromExpr((Expr *) node, var_colid_mapping, has_distributed_tables);
...
//---------------------------------------------------------------------------
//	@function:
//		CTranslatorScalarToDXL::CreateScalarWindowRef
//
//	@doc:
	...
	)
{
	...
	const WindowRef *window_ref = (WindowRef *) expr;
	...
	{
		ULONG *elem = mapping[ul];
		if ((ULONG) window_ref->winstage == elem[0])
		{
	...
	CDXLScalarWindowRef *winref_dxlop = GPOS_NEW(m_mp) CDXLScalarWindowRef
													(
													m_mp,
													GPOS_NEW(m_mp) CMDIdGPDB(window_ref->winfnoid),
													GPOS_NEW(m_mp) CMDIdGPDB(window_ref->restype),
													window_ref->windistinct,
													false,
													false,
													dxl_win_stage,
													win_spec_pos
	...
	TranslateScalarChildren(dxlnode, window_ref->args, var_colid_mapping);
	...
	)
{
	...
	if (1 == gpdb::ListLength(quals))
	{
		...
		return CreateScalarOpFromExpr(expr, var_colid_mapping, has_distributed_tables);
	}
	else
	{
	...
	)
{
	...
	dxlnode->AddChild(CreateScalarOpFromExpr(parrayref->refexpr, var_colid_mapping));

	if (NULL != parrayref->refassgnexpr)
	{
		dxlnode->AddChild(CreateScalarOpFromExpr(parrayref->refassgnexpr, var_colid_mapping));
	}
	...
	)
{
	...
	if (NULL == func_ptr)
	{
		...
		return CreateGenericCDXLDatum(mp, md_type, type_modifier, is_null, len, datum);
	}
	else
	{
	...
	)
{
	...
	BYTE *bytes = GetByteArray(mp, md_type, is_null, len, datum);
	ULONG length = 0;
	if (!is_null)
	{
	...
	)
{
	...
	CDXLDatum *datum_dxl = CTranslatorScalarToDXL::GetDatumVal(mp, md_type, gpmd::default_type_modifier, is_null, length, gpdb_datum);
