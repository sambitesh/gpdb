...
namespace gpdxl
{
	...
	class CTranslatorUtils
	{
		...
		public:
			...
			// check to see if the target list entry is a grouping column
			static
			BOOL IsGroupingColumn(const TargetEntry *target_entry, const GroupClause *group_clause);

			// check to see if the sorting column entry is a grouping column
			static
			BOOL IsGroupingColumn(const SortClause *sort_clause, List *group_clause_list);
